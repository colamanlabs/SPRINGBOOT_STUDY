...
public class MySimpleBatchConfig
{
    ...
    /*
     * page 106) 스프링배치에 포함된 커스텀 스텝 스코프와 잡 스코프를 사용하면 늦은 바인딩 기능을 쉽게 사용할 수 있다. 이 스코프 각각의 기능은 스탭의 실행 범위(스텝 스코프)나 잡의 실행범위(잡 스코프)에 들어갈 때 까지 빈 생성을 지연 시키는 것이다. 이렇게 함으로서 명령행
     * 또는 다른 소스에서 받아들인 잡 파라미터를 빈 생성 시점에 주입할 수 있다.
     * 
     * 
     * 
     * 
     * 스탭 스코프 선언 안하면 2024-02-16 00:10:32.985 INFO 2300 --- [ main] c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] retryCount:[null] 2024-02-16
     * 00:10:32.985 INFO 2300 --- [ main] c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] foo:[null] 2024-02-16 00:10:32.985 INFO 2300 --- [ main]
     * c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] name:[null] 2024-02-16 00:10:32.985 INFO 2300 --- [ main] c.c.s.s0007.MySimpleTasklet0001 :
     * [MySimpleTasklet0001/execute] requestDate:[null]
     * 
     * 
     * 스캡 스코프 선언하면 2024-02-16 00:13:21.337 INFO 15024 --- [ main] c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] retryCount:[11] 2024-02-16
     * 00:13:21.337 INFO 15024 --- [ main] c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] foo:[bar] 2024-02-16 00:13:21.337 INFO 15024 --- [ main]
     * c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] name:[basicJob] 2024-02-16 00:13:21.337 INFO 15024 --- [ main]
     * c.c.s.s0007.MySimpleTasklet0001 : [MySimpleTasklet0001/execute] requestDate:[20240215]
     */
    ...
    public Job createJob0001()
    {
         ...
         * 
         * https://javadoc.io/static/org.springframework.batch/spring-batch-core/4.3.5/org/springframework/batch/core/launch/support/RunIdIncrementer.html This
         * incrementer increments a "run.id" parameter of type Long from the given job parameters. If the parameter does not exist, it will be initialized to 1. The
         * parameter name can be configured using setKey(String).
         * 
    ...
    /*
     * JobBuilder 에서는 하나의 JobParameterValidator 지정만 가능하지만, CompositeJobParameterValidator 를 통해 복수개의 유효성 검증기 사용이 가능하다.
     * 
    ...
    public CompositeJobParametersValidator createValidator0001()
    {
         ...
         * 
         * DefaultJobParametersValidator(String[] requiredKeys, String[] optionalKeys) - Create a new validator with the required and optional job parameter keys
         * provided.
         * 
         * 필수파라미터 "requestDate" , run.id 만 추가
         */
        DefaultJobParametersValidator defaultJobParametersValidator = new DefaultJobParametersValidator(new String[]
        { "requestDate" }, new String[]
        { "run.id" });
        
        /*
         * afterPropertiesSet() 는 빈 초기화 이후에 호출되는 것인데, 왜 명시적으로 호출하는지 잘 모르겠다. 나중에 확인하자.
         */
